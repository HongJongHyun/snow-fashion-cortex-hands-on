-- ============================================================
-- EDU_SALES_SV Step 4: Verified Query (VQR) 추가
-- 
-- Step 1(Fact/Dimension) + Step 2(Description) + Step 3(Metric)에 이어서,
-- 비즈니스 문맥을 반영한 Verified Query를 추가합니다.
--
-- 변경 내용:
--   1) Fact/Dimension 수정 (Step 1)
--   2) 한국어 Description (Step 2)
--   3) METRICS 6개 (Step 3)
--   4) AI_VERIFIED_QUERIES 5개 추가:
--      BRAND_REVENUE, YOY_BRAND_GROWTH, STORE_EFFICIENCY,
--      VIP_CONTRIBUTION, WEEKDAY_PATTERN
--
-- 참고: 이 파일은 CREATE OR ALTER로 전체 정의를 다시 작성하므로,
--       EDU_SALES_SV_01, 02, 03의 변경 내용이 모두 포함되어 있습니다.
--       이 파일만 실행해도 Step 1+2+3+4가 모두 적용됩니다.
-- ============================================================
...
CREATE OR ALTER SEMANTIC VIEW SNOW_FASHION.SEMANTIC.EDU_SALES_SV
  ...
  COMMENT = '스노우패션 판매 분석용 Semantic View (교육용)'

  -- ============================================================
  -- AI_VERIFIED_QUERIES: 비즈니스 VQR
  -- ============================================================
  AI_VERIFIED_QUERIES (
    BRAND_REVENUE AS (
      QUESTION '브랜드별 총 매출은 얼마인가요?'
      VERIFIED_AT 1725408000
      ONBOARDING_QUESTION TRUE
      VERIFIED_BY '(STEWARD = admin)'
      SQL 'SELECT BRAND, SUM(SALE_AMOUNT) AS TOTAL_REVENUE FROM SNOW_FASHION.RAW.SALES_TRANSACTIONS GROUP BY BRAND ORDER BY TOTAL_REVENUE DESC'
    ),

    YOY_BRAND_GROWTH AS (
      QUESTION '브랜드별 전년 대비 매출 성장률은 어떻게 되나요?'
      VERIFIED_AT 1725408000
      ONBOARDING_QUESTION FALSE
      VERIFIED_BY '(STEWARD = admin)'
      SQL 'WITH yearly AS (SELECT BRAND, YEAR(TXN_DATE) AS YR, SUM(SALE_AMOUNT) AS REVENUE FROM SNOW_FASHION.RAW.SALES_TRANSACTIONS GROUP BY BRAND, YEAR(TXN_DATE)) SELECT cur.BRAND, cur.YR AS CUR_YEAR, cur.REVENUE AS CUR_REVENUE, prev.REVENUE AS PREV_REVENUE, ROUND((cur.REVENUE - prev.REVENUE) / NULLIF(prev.REVENUE, 0) * 100, 1) AS GROWTH_PCT FROM yearly cur LEFT JOIN yearly prev ON cur.BRAND = prev.BRAND AND cur.YR = prev.YR + 1 WHERE prev.REVENUE IS NOT NULL ORDER BY cur.YR DESC, GROWTH_PCT DESC'
    ),

    STORE_EFFICIENCY AS (
      QUESTION '매장별 평당 매출(평효율)은 어떻게 되나요?'
      VERIFIED_AT 1725408000
      ONBOARDING_QUESTION FALSE
      VERIFIED_BY '(STEWARD = admin)'
      SQL 'SELECT st.STORE_NAME, st.BRAND, st.REGION, st.AREA_SQM, SUM(s.SALE_AMOUNT) AS TOTAL_REVENUE, ROUND(SUM(s.SALE_AMOUNT) / NULLIF(st.AREA_SQM, 0)) AS REVENUE_PER_SQM FROM SNOW_FASHION.RAW.SALES_TRANSACTIONS s JOIN SNOW_FASHION.RAW.STORES st ON s.STORE_ID = st.STORE_ID GROUP BY st.STORE_NAME, st.BRAND, st.REGION, st.AREA_SQM ORDER BY REVENUE_PER_SQM DESC'
    ),

    VIP_CONTRIBUTION AS (
      QUESTION 'VIP 고객의 매출 비중은 얼마나 되나요?'
      VERIFIED_AT 1725408000
      ONBOARDING_QUESTION TRUE
      VERIFIED_BY '(STEWARD = admin)'
      SQL 'SELECT c.MEMBERSHIP_TIER, COUNT(DISTINCT s.CUSTOMER_ID) AS CUSTOMER_COUNT, SUM(s.SALE_AMOUNT) AS REVENUE, ROUND(SUM(s.SALE_AMOUNT) * 100.0 / SUM(SUM(s.SALE_AMOUNT)) OVER(), 1) AS REVENUE_SHARE_PCT FROM SNOW_FASHION.RAW.SALES_TRANSACTIONS s JOIN SNOW_FASHION.RAW.CUSTOMERS c ON s.CUSTOMER_ID = c.CUSTOMER_ID GROUP BY c.MEMBERSHIP_TIER ORDER BY REVENUE DESC'
    ),

    WEEKDAY_PATTERN AS (
      QUESTION '요일별 매출 패턴이 어떻게 되나요?'
      VERIFIED_AT 1725408000
      ONBOARDING_QUESTION FALSE
      VERIFIED_BY '(STEWARD = admin)'
      SQL 'SELECT DAYOFWEEK(TXN_DATE) AS DOW_NUM, DECODE(DAYOFWEEK(TXN_DATE), 0,''Sunday'', 1,''Monday'', 2,''Tuesday'', 3,''Wednesday'', 4,''Thursday'', 5,''Friday'', 6,''Saturday'') AS DAY_OF_WEEK, SUM(SALE_AMOUNT) AS REVENUE, COUNT(*) AS TXN_COUNT FROM SNOW_FASHION.RAW.SALES_TRANSACTIONS GROUP BY DOW_NUM, DAY_OF_WEEK ORDER BY DOW_NUM'
    )
  );
